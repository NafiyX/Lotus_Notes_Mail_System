...
class reminder_gui:
    def __init__(self):
        ...
        datetime_label = tkinter.Label(win, text="Record").grid(column=0,row=0)
        
        datetime_listbox = tkinter.Listbox(win, width=40)
        ...
        confirm_button = tkinter.Button(win, text="Confirm", command=onclick).grid(column=0, row=3)
        
        explanation_label = tkinter.Label(win, text="Please select a record and click ''Confirm'' to check the booking record's information.",
                                  font=20, justify=tkinter.CENTER)
